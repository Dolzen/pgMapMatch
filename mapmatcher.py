...
import pgmapmatch_py.tools as mmt

try:
    from pgmapmatch_py.config import *
except ModuleNotFoundError:
    raise Warning('config.py not found. Using config_template.py instead')
    from pgmapmatch_py.config_template import *


class traceCleaner():
    def __init__(self, traceTable, idName, geomNameOld, geomNameNew=None,
                 maxSpeed=maxSpeed, logFn=None,loglevel=logging.INFO):
        ...
        if logFn is not None:
            with open(self.logFn, 'a') as f:
                logging.debug('Logging to %s' % self.logFn)
    ...
    def getTracesDf(self, traceId=None, numLags=5):
        ...
        lagClause1 = ',\n'.join(
            ['ST_Distance(geom, l%(lag)s)/NULLIF(ST_M(l%(lag)s)-ST_M(geom),0)/1000*60*60 AS speed%(lag)s' % {'lag': lag}
             for lag in range(1, numLags + 1)])
        selectClause2 = '''\nFROM (SELECT traceid, (dp).path[1] AS ptid, (dp).geom AS geom,\n'''
        lagClause2 = ',\n'.join(
            ['lag((dp).geom,%(lag)s) OVER (PARTITION BY traceid ORDER BY (dp).path[1] DESC) AS l%(lag)s' % {'lag': lag}
             for lag in range(1, numLags + 1)])
    ...
    def fetchPtsToDrop(self, traceId=None, numLags=5):
        ...
        for gSize in range(1, numLags + 1):  # loop over groups of 1...5 points
            ...
            dropMask = ((df.lagSpeed > self.maxSpeed) & (df.speed1 > self.maxSpeed)) | (
                        (df.ptid == df.minPt + gSize - 1) & (df.speed1 > self.maxSpeed)) | (
                                   (df.lagSpeed > self.maxSpeed) & (df.ptid == df.maxPt))
    ...
    def dropPoints(self):
        ...
        if self.geomNameNew not in self.db.list_columns_in_table(self.traceTable):
            self.db.execute("SELECT AddGeometryColumn('%s','%s',%s,'LineStringM',3);" % (
            self.traceTable, self.geomNameNew, self.srid))
    ...
    def reportDroppedPoints(self):
        ...
        logging.debug('%s pts from %s traces in the drop list' % (len(self.ptsToDrop), nIds))
...
class mapMatcher():
    def __init__(self, streetsTable, traceTable=None, idName=None, geomName=None, newGeomName='matched_line',
                 cleanedGeomName=None, qualityModelFn='mapMatching_coefficients.txt', points=None, db=None, dummy_time=False):
        ...
        """
        logging.debug("Map matcher starting")
        if db is None:
        ...
        if float(self.pgr_version.split('.')[0]) < 2 or float(self.pgr_version.split('.')[0]) == 2 and float(
                self.pgr_version.split('.')[1]) < 3:
            raise Exception('pgRouting v2.3.0 or greater required to use pgr_dijkstraCostMatrix')
        self.safety_checks = False # Turn of for time consuming safety checks
        ...
        logging.debug("Checking SRID")
        self.streets_srid = str(
            self.db.execfetch('SELECT ST_SRID(%s) FROM %s LIMIT 1;' % (streetGeomCol, streetsTable))[0][0])
        if self.streets_srid == '4326':
            raise Exception(
                'Streets SRID is 4326. You should use a projected coordinate system. Project your streets geometry column and try again.')
        if traceTable is not None:
            self.trace_srid = str(
                self.db.execfetch('SELECT ST_Srid(%s) FROM %s LIMIT 1' % (self.geomName, self.traceTable))[0][0])
        ...
        if points is not None:
            self.create_lat_lon(points,dummy_time)
            self.cmdDict['traceLineStr'] = self.traceLineStr
            self.cmdDict['streets_srid'] = self.streets_srid
            #logging.debug(self.traceLineStr)
            buffer_env_query = '''SELECT ST_ASText(ST_BUFFER(%(traceLineStr)s,%(bounding_buffer)s))''' % self.cmdDict
            env_query = '''SELECT ST_ASText(ST_ENVELOPE(ST_BUFFER(ST_ENVELOPE(%(traceLineStr)s),100.0)))''' % self.cmdDict
            #logging.debug(env_query)
            text_envelope = self.db.execfetch(env_query)[0][0]
            #logging.debug(text_envelope)
            self.cmdDict['text_envelope'] = text_envelope
            envelope_filter = """ST_GeomFromText('%s',%s)""" % (self.cmdDict['text_envelope'],self.cmdDict['streets_srid'])
            #logging.debug(envelope_filter)
            self.traceLineEnv = envelope_filter
            self.cmdDict['traceLineEnv'] = self.traceLineEnv
            filtered_streets_table = 'select * from %(streetsTable)s where %(streetGeomCol)s && %(traceLineEnv)s' % self.cmdDict
            t = self.cmdDict['text_envelope']
            t = t.strip('POLYGON').strip('(').strip(')').split(',')
            nt = ','.join(
                ['ST_SetSRID(ST_MakePoint({p}),{srid})'.format(p=i.replace(' ', ','), srid=self.cmdDict['streets_srid'])
                 for i in
                 t])
            filter = 'ST_MakePolygon(ST_MakeLine(ARRAY[{format_points}]))'.format(format_points=nt)
            self.cmdDict['dijkstra_where_clause'] = 'where geom && %s' % filter
        else:
            filtered_streets_table = 'select * from   %(streetsTable)s where %(streetGeomCol)s && ' \
                                     '(select ST_buffer(ST_ENVELOPE(%(geomName)s),0.002) from %(traceTable)s))' % self.cmdDict

        self.cmdDict['filtered_streets_table'] = filtered_streets_table

        fwayColList = [cc.strip() for cc in fwayCols.split(',')]

        # Check geometry types of streets
        if self.safety_checks:
            logging.debug('Checking Geometry of Road Network')
            cmd =  '''SELECT DISTINCT ST_GeometryType(%(streetGeomCol)s) FROM %(streetsTable)s;''' % self.cmdDict #This is very expensive when using large tables
            #logging.debug(cmd)
            result = self.db.execfetch(cmd)
            if any([rr[0] == 'ST_MultiLineString' for rr in result]):
                raise Exception(
                    'Streets geometry column %s must be ST_LineString, not ST_MultiLineString. Use ST_LineMerge() to convert your table.')
            ...
            logging.debug('Checking Length of table table')
            nsts = self.db.execfetch('''SELECT COUNT(%(streetIdCol)s) FROM %(streetsTable)s;''' % self.cmdDict)
            if nsts[0][0] > 1000000:
                logging.debug(
                    'Streets table has %d rows, which may impair pgrouting performance. Consider clipping the table to your area of interest.' %
                    nsts[0][0])

            # Check geometry types of trace table
            logging.debug('Checking Trace table')
            if traceTable is not None:
        ...
        # Get all streets????
        logging.debug('Getting all streets')
        if traceTable is not None:  # tables get input preference
            # THIS LINE IS BROKE
            cmd = '''SELECT %(streetIdCol)s, %(source)s::integer, %(target)s::integer, %(cost)s::real, %(reverse_cost)s::real, %(km)s::real, 
                %(kmh)s::real %(fwayCols)s FROM %(streetsTable)s where %(streetGeomCol)s && (select ST_buffer(ST_ENVELOPE(%(geomName)s),0.002) from %(traceTable)s)''' % self.cmdDict
        elif points is not None:
            cmd = '''SELECT %(streetIdCol)s, %(source)s::integer, %(target)s::integer, %(cost)s::real, %(reverse_cost)s::real, %(km)s::real,
                %(kmh)s::real %(fwayCols)s FROM %(streetsTable)s where %(streetGeomCol)s && %(traceLineEnv)s''' % self.cmdDict
        else:
            raise ValueError
        #logging.debug(cmd)
        edgesDf = pd.DataFrame(self.db.execfetch(cmd),
                               columns=['edge', 'source', 'target', 'cost', 'reverse_cost', 'km',
        ...
        logging.debug('maxN', maxN)
        logging.debug('edge table', edgesDf.shape)
    ...
    def create_lat_lon(self, points,dummy=False):
        # Create appropriate datastructure
        traceSegment = {'points': []}
        for i in range(len(points)):
            p = {'latitude': points[i][0],
                 'longitude': points[i][1],
                 'time': points[i][2]}
            traceSegment['points'].append(p)
        if not dummy:
            self.traceLineStr = 'ST_Transform(ST_SetSRID(ST_MakeLine(ARRAY[' + ', '.join([
                'ST_MakePointM(' + str(pt['longitude']) + ',' + str(pt['latitude']) + ',' +
                str(int((pt['time'].replace(tzinfo=None) - datetime.datetime(1970, 1, 1)).total_seconds())) + ')'
    ...
    def matchLatLon(self, points,dummy=False):
        self.create_lat_lon(points,dummy)
        self.matchTrace()

    def matchTrace(self):
        """New version using Viterbi"""
        logging.debug("Starting matchTrace()")
        ...
        if len(self.nids) < 3:
            self.matchStatus = 1
            logging.debug('Skipping match. Need at least 3 points in trace')
            return
        #logging.debug('Updating cost matrix')
        ...
        for nid1, rr1 in self.ptsDf.loc[:self.nids[-2]].iterrows():
            ...
            nid_count += 1
            # logging.debug('nid count',nid_count,'nid',nid1,rr1)
            for dir1 in [0, -1]:
                ...
                # logging.debug(self.ptsDf.loc[nid1+1:nid1+1+max_skip].drop_duplicates('edge'))
                # logging.debug('inner loop size',self.ptsDf.loc[nid1+1:nid1+1+max_skip].drop_duplicates('edge').shape)
                for nid2, rr2 in self.ptsDf.loc[nid1 + 1:nid1 + 1 + max_skip].drop_duplicates(
        ...
        self.temporalScores = sparse.coo_matrix(([ii[0] for ii in scores], (rowKeys, colKeys)), shape=(self.N, self.N),
                                                dtype=np.float32).tocsr()
        self.topologicalScores = sparse.coo_matrix(([ii[1] for ii in scores], (rowKeys, colKeys)),
                                                   shape=(self.N, self.N), dtype=np.float32).tocsr()
        self.uturns = sparse.coo_matrix(([ii[2] for ii in scores], (rowKeys, colKeys)), shape=(self.N, self.N),
                                        dtype=np.int8).tocsr()
        self.timing['iterator'] += (time.time() - starttime)

        logging.debug("Starting viterbi")
        starttime = time.time()
        d = self.viterbi()
        self.timing['decode'] += (time.time() - starttime)
        logging.debug('Done viterbi')
        route = [(self.ptsDf.loc[self.ptsDf.rownum == int(dd / 2), 'edge'].values[0], -1 * dd % 2) for dd in
        ...
        distLLs = self.ptsDf[self.ptsDf.rownum.isin([int(dd / 2) for dd in d])].distprob.describe()[
            ['mean', 'min']].tolist()
        distLLs[0] = distLLs[0] / weight_1stlast
        distLLs[-1] = distLLs[-1] / weight_1stlast
        logging.debug('Calculating temporal and topological arrays')
        ...
        #===================???=========================
        self.LL = distLLs + [temporalLLarray.mean(), temporalLLarray.min(), topologicalLLarray.mean(),
                             topologicalLLarray.min()]  #How is this ACTUALLY USED?????
        #===================???=========================
        self.matchStatus = 0
    ...
    def getPtsDf(self):
        ...
        if self.traceId:  # get trace from postgres
            logging.debug("Getting Points using trace")
            dpStr = 'SELECT ST_DumpPoints(%(geomName)s) AS dp FROM %(traceTable)s WHERE %(idName)s=%(traceId)s' % dict(
                self.cmdDict, **{'traceId': self.traceId})
            pre_filter = 'WITH  filter_data as (select %(streetGeomCol)s,%(streetIdCol)s, ' \
                         '%(fwayQuery)s from %(streetsTable)s where %(streetGeomCol)s && ' \
                         '(select ST_buffer(ST_ENVELOPE(%(geomName)s),0.002) from %(traceTable)s)) ' %dict(self.cmdDict, **{
                'fwayQuery': fwayCols})
        else:  # trace was a GPX file
            dpStr = 'SELECT ST_DumpPoints(%s) AS dp' % self.traceLineStr
            pre_filter = 'WITH filter_data as (select %(streetGeomCol)s,%(streetIdCol)s, %(fwayQuery)s from %(streetsTable)s where %(streetGeomCol)s && ' \
                         '%(traceLineEnv)s) ' % dict(self.cmdDict, **{
                'traceLineStr': self.traceLineStr,'fwayQuery': fwayCols})
            #logging.debug(pre_filter)
        ...
        cmd = '''SELECT (dp).path[1]-1 AS path, %(streetIdCol)s, ST_Distance((dp).geom, %(streetGeomCol)s),
                 ...
                 FROM filter_data, (%(dpStr)s) AS pts
        ...
        cmd = pre_filter + cmd
        logging.debug('Get PTS command', cmd[:30],'....')
        pts = self.db.execfetch(cmd)
        if pts is None or pts == []:
            self.ptsDf, self.nids = None, []
            logging.debug('No streets found within the tolerance of the trace.')
            logging.debug(
        ...
        nidStr = str(tuple(self.ptsDf.nid.unique())) if len(self.ptsDf.nid.unique()) > 1 else '(' + str(
            self.ptsDf.nid.iloc[0]) + ')'
        ...
        self.ptsDf = self.ptsDf.join(
            pd.DataFrame(traceSegs, columns=['nid', 'seglength']).set_index('nid')).sort_index()
        ...
        logging.debug('Number of points', self.ptsDf.shape)
        self.timing['getPoints'] += (time.time() - starttime)

    def writeMatchToPostgres(self, edgeIdCol='edge_ids', writeEdgeIds=True, writeGeom=True, writeMatchScore=True,
                             writeLLs=False):
        ...
        if writeLLs: cols += [('ll_dist_mean', 'real'), ('ll_dist_min', 'real'), ('ll_topol_mean', 'real'),
                              ('ll_topol_min', 'real'), ('ll_distratio_mean', 'real'), ('ll_distratio_min', 'real')]
        ...
        if writeEdgeIds or writeMatchScore or writeLLs:
            ...
            if writeMatchScore and not np.isnan(match_score): cmd += 'match_score=%s,' % np.float32(
                match_score)  # float32 needed to avoid underflow with very low scores
    ...
    def writeGeomToPostgres(self):
        ...
        if self.newGeomName not in self.db.list_columns_in_table(self.traceTable):
            self.db.execute("SELECT AddGeometryColumn('%s','%s',%s,'LineString',2);" % (
            self.traceTable, self.newGeomName, self.trace_srid))
        if self.cleanedGeomName is not None and self.cleanedGeomName not in self.db.list_columns_in_table(
                self.traceTable):
            self.db.execute("SELECT AddGeometryColumn('%s','%s',%s,'LineStringM',3);" % (
            self.traceTable, self.cleanedGeomName, self.trace_srid))
        ...
        if self.matchedLineString is not None:
            cmd = '''UPDATE %(traceTable)s SET %(newGeomName)s = geom FROM (%(matchedLineStr)s) q
                     WHERE %(idName)s = %(traceId)s;''' % dict(self.cmdDict,
                                                               **{'matchedLineStr': self.matchedLineString,
    ...
    def getMatchedLineString(self):
        ...
        if len(route) == 1:  # single edge to match
            cDict = dict(self.cmdDict,
                         **{'edge': str(route[0]), 'traceId': self.traceId, 'startGeom': self.startEndPts[0],
                            'endGeom': self.startEndPts[1],
        ...
        prevNode = self.edgesDf.source[route[0]] if self.edgesDf.target[route[0]] in self.edgesDf.ix[route[1]][
            ['source', 'target']].tolist() else self.edgesDf.target[route[0]]
            ...
            if ii == 0:  # first point - don't need whole edge
                if reverse:
                    geomField = 'ST_LineSubString(%(streetGeomCol)s, 0, ST_LineLocatePoint(%(streetGeomCol)s, %(startGeom)s))' % dict(
                        self.cmdDict, **{'startGeom': self.startEndPts[0]})
                else:
                    geomField = 'ST_LineSubString(%(streetGeomCol)s, ST_LineLocatePoint(%(streetGeomCol)s, %(startGeom)s), 1)' % dict(
                        self.cmdDict, **{'startGeom': self.startEndPts[0]})
                routeFrcs = None
                extra = ', %s WHERE %s=%s AND' % (
                self.traceTable, self.idName, self.traceId) if self.traceId is not None else ' WHERE '
                ...
                if reverse:
                    geomField = 'ST_LineSubString(%(streetGeomCol)s, ST_LineLocatePoint(%(streetGeomCol)s, %(endGeom)s), 1)' % dict(
                        self.cmdDict, **{'endGeom': self.startEndPts[1]})
                else:
                ...
                if edge == route[ii + 1] and edge != route[ii - 1] and (ii + 2 == len(route) or edge != route[
                    ii + 2]):  # first edge in uTurn, but not if we have a triple
                    # uturnFrcs==-1 means an unexpected Uturn
                    routeFrcs = None if self.uturnFrcs[ii] == -1 else (self.uturnFrcs[ii][0], 1) if reverse else (
                    0, self.uturnFrcs[ii][1])
                    if routeFrcs is None: logging.debug('Warning. Missing Uturn in trace %s' % self.traceId)
                elif edge == route[ii - 1] and (ii == 1 or edge != route[ii - 2]):  # second edge in uTurn
                    routeFrcs = None if self.uturnFrcs[ii - 1] == -1 else (
                    0, self.uturnFrcs[ii - 1][1]) if reverse else (self.uturnFrcs[ii - 1][0], 1)
                    if routeFrcs is None: logging.debug('Warning. Missing Uturn in trace %s' % self.traceId)
                else:
                    routeFrcs = None
    ...
    def writeCleanedGeom(self):
        ...
        cmd = 'UPDATE %s SET %s = OLDGEOM WHERE %s = %s;' % (
        self.traceTable, self.cleanedGeomName, self.idName, self.traceId)
    ...
    def getMatchScore(self):
        ...
        if 'gpsMatchRatio' in self.qualityModelCoeffs or 'matchGpsRatio' in self.qualityModelCoeffs:
            ...
            gpslength, matchlength = self.db.execfetch('''SELECT ST_Length(%(geomToUse)s), ST_Length(%(newGeomName)s
                                                             FROM %(traceTable)s WHERE trip_id=%(id)s;''' % dict(
                self.cmdDict, **{'geomToUse': geomToUse, 'id': self.traceId}))
    ...
    def loadQualityModel(self):
        ...
        self.qualityModelCoeffs = dict(
            [(cl.split()[0], float(cl.split()[1])) for cl in coeffLines if len(cl.split()) == 2])
        knownCols = ['intercept', 'frechet_dist', 'll_dist_mean', 'll_dist_min', 'll_topol_mean', 'll_topol_min',
                     'll_distratio_mean', 'll_distratio_min', 'gpsMatchRatio', 'matchGpsRatio']
    ...
    def local_dijkstra_graph(self):
        pass
        #Create the local dijkstra graph
        query = 'SELECT %(streetIdCol)s as id, %(source)s, %(target)s, %(cost)s as cost, %(reverse_cost)s as rcost FROM %(streetsTable)s  %(dijkstra_where_clause)s'  % self.cmdDict
        logging.debug(query)
                    ...
                    if nodesToDo_src and nodesToDo_tgt:
                        ...
                        cmd = '''SELECT start_vid, end_vid, sum(pgr.cost) AS pgrcost, sum(s.km) AS length_km
                              FROM %(streetsTable)s s,
                                   pgr_dijkstra('SELECT %(streetIdCol)s as id, %(source)s, %(target)s, %(cost)s as cost, %(reverse_cost)s as rcost FROM %(streetsTable)s %(dijkstra_where_clause)s',
                                                 ARRAY%(srcnodes)s, ARRAY%(tgtnodes)s, True) AS pgr
                              WHERE s.%(streetIdCol)s=pgr.edge
                              GROUP BY start_vid,end_vid;''' % dict(self.cmdDict,
                                                                    **{'srcnodes': str(list(nodesToDo_src)),
                                                                       'tgtnodes': str(list(nodesToDo_tgt)),
                                                                       'filter':filter,
                                                                        })
                        #logging.debug(cmd)


                        # logging.debug("Dijkstra Nodes ", cmd)
                        result = self.db.execfetch(cmd)
                        self.costMatrix.update(
    ...
    def fillRouteGaps(self, route, uturns):
        ...
        for ii, (edge, node, uturn, ptId) in enumerate(zip(edgeList[1:], nodeList[1:], uturnList, keepPts[1:-1])):
            ...
            if uturn in [3, 4]:  # uturn on first edge
                ...
                frcs = self.ptsDf.loc[keepPts[ii]:keepPts[ii + 1] - 1].loc[
                    self.ptsDf.loc[keepPts[ii]:keepPts[ii + 1] - 1].edge == route[ptId - 1][0], 'frcalong']
                uturnFrcs = uturnFrcs[:-1] + [(frcs.min(), frcs.max()), -1]
                oNode = self.edgesDf.target[fullroute[-1]] if self.edgesDf.source[fullroute[-1]] == oNode else \
                self.edgesDf.source[fullroute[-1]]
            ...
            if oNode != dNode:  # missing edges
                ...
                cmd = '''SELECT array_agg(edge) AS edges FROM pgr_dijkstra(
                            'SELECT %(streetIdCol)s as id, %(source)s, %(target)s, %(cost)s as cost, %(reverse_cost)s as reverse_cost FROM %(streetsTable)s %(dijkstra_where_clause)s', %(oNode)s, %(dNode)s, True);
                      ''' % dict(self.cmdDict, **{'oNode': str(oNode), 'dNode': str(dNode)})
                #logging.debug('fillRouteGaps cmd',cmd)
                result = self.db.execfetch(cmd)
                if result[0][0] is None:  # because of a disconnected network/islands
                    logging.debug('Could not match trace. Network may be disconnected or have islands.')
                    logging.debug(cmd)
                    self.bestRoute = None
                    self.uturnFrcs = None
                    return
            ...
            if uturn in [2, 4]:
                ...
                frcs = self.ptsDf.loc[keepPts[ii + 1]:keepPts[ii + 2]].loc[
                    self.ptsDf.loc[keepPts[ii + 1]:keepPts[ii + 2]].edge == route[ptId][0], 'frcalong']
                uturnFrcs.append((frcs.min(), frcs.max()))
            fullroute.append(edge)
            if uturn == 1:
                frcs = self.ptsDf.loc[keepPts[ii]:keepPts[ii + 2] - 1].loc[
                    self.ptsDf.loc[keepPts[ii]:keepPts[ii + 2] - 1].edge == route[ptId][0], 'frcalong']
    ...
    def cleanupRoute(self, d):
        ...
        while (len(self.bestRoute) > 1 and
               (lastEdgeLength * frc < 0.005 or (
                       self.allowFinalUturn is False and self.bestRoute[-1] == self.bestRoute[-2]))):
    ...
    def addQualityColumns(self, columns=None, forceUpdate=False):
        ...
        if colsToAdd[0][0] in self.db.list_columns_in_table(self.traceTable):
            if forceUpdate:
                ...
                logging.debug('Quality columns already added. Skipping')
                return
        ...
        if columns is None or 'frechet_dist' in columns:
            ...
            ids = sorted([ii[0] for ii in self.db.execfetch(
                'SELECT %(idName)s FROM %(traceTable)s WHERE %(newGeomName)s IS NOT NULL' % self.cmdDict)])
            for id in ids:
                self.traceId = id
                fd = self.frechet()
    ...
    def transProbSameEdge(self, rr1, rr2, dir, sl):
        # logging.debug('calc transprob same edge')
    ...
    def transProb(self, rr1, rr2, dir1, dir2, sl):
        ...
        cratio = (e1cost * frc1 + rc + self.uturncost * (uturn > 0) + self.uturncost * 2 * (
    ...
    def viterbi(self):
        ...
        for nid in self.nids[1:]:
            nToSkip = 0 if nid == len(
                self.nids) - 1 else max_skip  # Don't allow last point to be dropped, so set other probabilities very small
            ...
            trellis_1 = np.nanmax(np.broadcast_to(trellis_0[:, None], (trellis_0.shape[0], iterN)) +
                                  (np.repeat(self.ptsDf.loc[nid - nToSkip:nid, 'distprob'],
                                             2) - self.skip_penalty * np.repeat(
                                      np.array(nid - self.ptsDf.loc[nid - nToSkip:nid].index) ** 2, 2))[:, None].T +
                                  LL, 0)

            backpt[idx1:idx2, nid] = np.nanargmax(np.broadcast_to(trellis_0[:, None], (trellis_0.shape[0], iterN)) + LL,
                                                  0)[:, None] + lastidx1 + 1
    ...
    def frechet(self, resolution=20):
        ...
        if self.traceId:
            ...
            traceLength = self.db.execfetch(
                'SELECT ST_Length(%(frechetGeomName)s) FROM %(traceTable)s WHERE %(idName)s=%(traceId)s;' % dict(
                    self.cmdDict, **{'traceId': self.traceId, 'frechetGeomName': frechetGeomName}))[0][0]
            cmd = '''WITH l AS (SELECT ST_Force2D(%(newGeomName)s) as l1,
                                       ST_Force2D(%(frechetGeomName)s) as l2
                                 FROM %(traceTable)s where %(idName)s=%(traceId)s)''' % dict(self.cmdDict,
                                                                                             **{'traceId': self.traceId,
                                                                                                'frechetGeomName': frechetGeomName})
        else:
            ...
            self.traceLineStr, self.matchedLineString)

        frc = max(0.0025,
                  resolution * 1. / traceLength)  # max is because of limit of target lists can have at most 1664 entries

        cmd += '\nSELECT ' + ','.join([' ST_AsText(ST_LineInterpolatePoint(l1,' + str(
            f) + ')), ST_AsText(ST_LineInterpolatePoint(l2,' + str(f) + '))' for f in np.arange(0, 1, frc)])
        cmd += ' FROM l'
        try:
...
class qualityPredictor():
    ...
    def getPostgresData(self, table=None, estSample=True):
        ...
        if estSample:
            cmd = """SELECT %(idName)s, match_good, ST_NPoints(%(geomName)s), %(cols)s
                        FROM %(table)s
                        WHERE matched_line is Not Null AND match_good!='drop' AND match_good is not Null""" % dict(
                self.cmdDict, **{'table': table, 'cols': cols})
            df = pd.DataFrame(self.db.execfetch(cmd),
                              columns=['trip_id', 'match_good', 'num_pings'] + cols.split(', ')).set_index('trip_id')
            ...
            df = pd.DataFrame(self.db.execfetch(cmd), columns=['trip_id', 'num_pings'] + cols.split(', ')).set_index(
                'trip_id')
    ...
    def estimateQualityModel(self, holdback=0.3, threshold_score=0.7):
        ...
        for model in ['model1', 'model2', 'model3']:
            ...
            if model == modelToUse: logging.debug('\nThis is the preferred model.')
            ...
            dfRes.loc[maskRes, 'pr_good'] = ro.r.predict(logitmodel, newdata=dfRes[maskRes].to_records(),
                                                         type='response')
            dfRes.loc[:, 'pred_good'] = dfRes.pr_good >= threshold_score

            xtabEst = pd.crosstab(dfEst[mask].match_good.astype(bool), dfEst[mask].pred_good, rownames=['Observed'],
                                  colnames=['Predicted (regression)'])
            self.writeLog(xtabEst.to_string() + '\n')
            self.writeLog('Hit rate: %.2f' % ((xtabEst[0][0] + xtabEst[1][1]) * 1. / xtabEst.sum().sum()))

            xtabRes = pd.crosstab(dfRes[maskRes].match_good.astype(bool), dfRes[maskRes].pred_good,
                                  rownames=['Observed'], colnames=['Predicted (regression)'])
            ...
            if model == modelToUse:
                ...
                with open(self.outLogitCoeffsFn, 'w') as output_file:
                    for ii in range(fit.rx2('coefficients').nrow):
                        output_file.write(
                            fit.rx2('coefficients').rownames[ii] + '\t' + str(fit.rx2('coefficients')[ii]) + '\n')
    ...
    def predictQuality(self, table=None):
        ...
        logging.debug('Loading original fit')
        ...
        df.loc[mask, 'pr_good'] = ro.r.predict(logitmodel, newdata=df[mask][varsToUse].to_records(),
                                               type='response')  # Convert to record array to use in R

        # Write back to the database
        logging.debug('Writing fits to temp table')
        ...
        logging.debug('Merging and cleaning up')
        ...
        self.db.execute(
            'UPDATE %(table)s t1 SET pr_good = t2.pr_good FROM tmpimport AS t2 WHERE t1.%(idName)s=t2.%(idName)s;' % dict(
                self.cmdDict, **{'table': table}))
